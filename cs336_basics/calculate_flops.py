...
def calculate_learnable_params(vocab_size, context_length, num_layers, d_model, num_heads, d_ff):
    ...
    layers = num_layers * (4 * (d_model * d_model) + 2 * d_model + 3 * (d_model * d_ff))
...
def main():
    ...
    print(f"Total number of trainable params: {t_p} ({int(t_p * 1e-6)} M)")
    ...
    print("large")
    t_p = calculate_learnable_params(**GPT_2_LARGE)
    print(f"Total number of trainable params: {t_p} ({int(t_p * 1e-6)} M)")
    print(f"Memory {t_p * 4 / 1e9} GB")
